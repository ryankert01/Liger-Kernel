...
from liger_kernel.chunked_loss import LigerFusedLinearKLDivLoss
from liger_kernel.chunked_loss.functional import liger_fused_linear_kl_div
from liger_kernel.chunked_loss.kl_div_loss import LigerFusedLinearKLDivFunction
...
class HFKLDivLoss(HFDistillationLoss):
    """
    Naive implementation of a distillation loss using KL Divergence.
    """
...
class LigerLMHeadKLDiv(torch.nn.Module):
    def __init__(
        ...
        self.student_lin = torch.nn.Linear(in_features=H // 2, out_features=V, bias=False, dtype=dtype, device=device)
        self.teacher_lin = torch.nn.Linear(in_features=H, out_features=V, bias=False, dtype=dtype, device=device)
        self.chunked_kl_div = LigerFusedLinearKLDivLoss(
            weight_hard_loss=weight_hard_loss,
            weight_soft_loss=weight_soft_loss,
            ignore_index=ignore_index,
            temperature=temperature,
...
def test_correctness(
    ...
    torch_lm_head_kl_div = TorchLMHeadKLDiv(
        H=H,
        V=V,
        dtype=dtype,
        device=device,
    ...
    liger_lm_head_kl_div = LigerLMHeadKLDiv(
        H=H,
        V=V,
        dtype=dtype,
        device=device,
    ...
    torch_lm_head_kl_div.student_lin.weight.data = liger_lm_head_kl_div.student_lin.weight.data = torch.rand(
        V, H // 2, device=device, dtype=dtype
    )
    torch_lm_head_kl_div.teacher_lin.weight.data = liger_lm_head_kl_div.teacher_lin.weight.data = torch.rand(
    ...
    loss1 = torch_lm_head_kl_div(student_input1, teacher_input, target)
    loss2 = liger_lm_head_kl_div(student_input2, teacher_input, target)
    ...
    assert_verbose_allclose(
        torch_lm_head_kl_div.student_lin.weight.grad,
        liger_lm_head_kl_div.student_lin.weight.grad,
        atol=atol,
        rtol=rtol,
    )
